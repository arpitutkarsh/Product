...
const AddProduct = () => {
  ...
  const handleFileChange = (e) => {
    ...
    console.log(`📌 Selected files for ${name}:`, files);
    ...
    return (
      <div className="flex flex-wrap gap-4 mt-3">
        {files.map((file, index) => (
          <div key={index} className="relative group border rounded-xl overflow-hidden shadow-lg hover:scale-105 transition-transform duration-300">
            {type === "image" ? (
        ...
        <div className="flex flex-1 justify-center items-start p-8">
          <div className="w-full max-w-3xl">
            <form onSubmit={handleSubmit} className="bg-white rounded-3xl shadow-2xl p-8 border border-gray-200">
              ...
              <button type="submit" disabled={loading} className={`mt-8 w-full py-3 rounded-2xl text-white font-bold text-lg transition shadow-lg ${loading ? "bg-purple-400 cursor-not-allowed" : "bg-gradient-to-r from-purple-600 to-pink-500 hover:from-purple-700 hover:to-pink-600"}`}>
